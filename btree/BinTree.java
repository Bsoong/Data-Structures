...
//	height(Node(i,lt,rt)) = 1 + max(height(lt),height(rt))
	...
	private boolean isBalancedHelp(Node<E> n) {
		if(n == null) {
			return true;
		} else {
			return Math.abs(height(n.left) - height(n.right)) <= 1 && isBalancedHelp(n.left) && isBalancedHelp(n.right);
		}
	}
	...
	private int noOfLeaveshelp(Node<E> n) {
		if(n == null) {
			return 0;
		} else {
			if(n.left == null && n.right == null) {
				...
				return noOfLeaveshelp(n.left) + noOfLeaveshelp(n.right);
			}
		}
	}
	public int noOfLeaves() {
		return noOfLeaveshelp(root);
	}
	...
	public void prune(int level, Node<E> n) {
		if(n == null) {
			n.left = null;
			n.right = null;
		} else {

...
public class MyListLL<E> {
	...
	public static zip Node(a(E,F)) zip(Node (E) list1, Node(F) list2) {
		Node<Pair<E,F>> order = new Node<Pair<E,F>>;
		while(list1.next != null || list2.next!= null) {
			current = list1.data;
			current2 = list2.data;
			if(current.compareTo(current2) == -1) {
				order.add(current1);
			}
			else if(current.compareTo(current2) == 0) {
				System.out.println("Error same numbers in the sets");
			}
			else {
				order.add(current2);
			}
